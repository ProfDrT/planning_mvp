...
# Wrap imports in try-except blocks to handle potential import errors
try:
    ...
    if input_type == "Staff":
        ...
        if st.button("Add Staff"):
            try:
                ...
                st.error(f"Failed to add staff: {str(e)}")
        ...
        if st.button("Add Equipment"):
            try:
                ...
                st.error(f"Failed to add equipment: {str(e)}")
...
def view_resources_page():
    ...
    try:
        ...
        st.error(f"Failed to view resources: {str(e)}")
        st.error(traceback.format_exc())
